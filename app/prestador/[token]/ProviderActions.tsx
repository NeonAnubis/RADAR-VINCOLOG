...
const STATUS_BUTTONS = [
  { key: 'em_rota_coleta',       label: 'Em rota para coleta',  icon: Truck,       color: '#60A5FA' },
  { key: 'chegou_coleta',        label: 'Cheguei na coleta',     icon: MapPin,      color: '#60A5FA' },
  { key: 'carregamento_iniciado',label: 'Carregamento iniciado', icon: Package,     color: '#38BDF8' },
  { key: 'carga_embarcada',      label: 'Carga embarcada',       icon: Package,     color: '#38BDF8' },
  { key: 'saiu_coleta',          label: 'Saí da coleta',         icon: Truck,       color: '#A78BFA' },
  { key: 'em_transito',          label: 'Em trânsito',           icon: Truck,       color: '#A78BFA' },
  { key: 'chegou_destino',       label: 'Cheguei no destino',    icon: MapPin,      color: '#F472B6' },
  { key: 'descarga_iniciada',    label: 'Descarga iniciada',     icon: Package,     color: '#FB923C' },
] as const
...
export default function ProviderActions({ token, oit }: { token: string; oit: DbOit }) {
  ...
  function handlePodSubmit(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!podName.trim()) { alert('Informe o nome do recebedor'); return }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="glass rounded-2xl p-4">
        <p className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-3">Atualizar Status</p>
          ...
          {STATUS_BUTTONS.map(b => {
            ...
            return (
              <div key={b.key} className="space-y-1.5">
                <button onClick={() => handleStatus(b.key, b.label)} disabled={pending}
      ...
      <div className="rounded-2xl p-4" style={{ background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.25)' }}>
        {!showPod ? (
          <button onClick={() => setShowPod(true)}
            ...
            <CheckCircle className="w-4 h-4" /> Entrega Concluída + POD
          </button>
        ) : (
          <div className="space-y-3">
            <p className="text-sm font-bold text-emerald-300">📋 Registrar POD / Comprovante de Entrega</p>
            <div>
              <label className="block text-xs font-bold text-blue-300 mb-1 uppercase tracking-wider">Nome do recebedor *</label>
              <input value={podName} onChange={e => setPodName(e.target.value)} placeholder="Ex: João Silva" className="glass-input" />
            </div>
            <label className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-bold text-white cursor-pointer"
              ...
              {pending ? 'Enviando...' : 'Tirar foto do canhoto/POD'}
              <input type="file" accept="image/*" capture="environment" className="hidden" onChange={handlePodSubmit} />
            </label>
            <button onClick={() => setShowPod(false)} className="w-full text-xs font-medium text-blue-400">Cancelar</button>
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl p-4" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)' }}>
        {!showOccurrence ? (
          ...
          <div className="space-y-3">
            <p className="text-sm font-bold text-red-300">⚠ Reportar Ocorrência</p>
            ...
            <textarea value={occDesc} onChange={e => setOccDesc(e.target.value)} rows={3} placeholder="Descreva a ocorrência..." className="glass-input resize-none" />
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowOccurrence(false)} className="px-4 py-2.5 rounded-xl text-sm font-medium text-blue-400 glass">Cancelar</button>
            </div>
          </div>
        )}
      </div>
      ...
      {oit.contract_pdf_url && !oit.signed_contract_url && (
        <div className="rounded-2xl p-4" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(96,165,250,0.25)' }}>
          <p className="text-sm font-bold text-blue-300 mb-2 flex items-center gap-1.5"><FileSignature className="w-4 h-4" /> Contrato Pendente</p>
          <p className="text-xs text-blue-400">Confirme com a operação VINCOLOG quando assinar o contrato.</p>
        </div>
      )}
    </div>
  )
