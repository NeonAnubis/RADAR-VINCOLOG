...
import { Users, Star, Truck, MapPin } from 'lucide-react'
...
export default async function DashboardPrestadoresPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Dashboard Prestadores</h1>
        <p className="text-blue-400 mt-0.5 text-sm">Visão gerencial dos prestadores cadastrados</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Metric label="Total cadastrados" value={all.length}   color="bg-blue-500/25 text-blue-300" icon={Users} />
        <Metric label="Ativos"            value={ativos}       color="bg-emerald-500/25 text-emerald-300" icon={Users} />
        <Metric label="Adormecidos"       value={adormecidos}  color="bg-slate-500/25 text-slate-300" icon={Users} />
        <Metric label="Fretes realizados" value={totalFretes}  color="bg-violet-500/25 text-violet-300" icon={Truck} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="glass rounded-2xl p-5">
          <h2 className="font-bold text-white mb-4">Por Tipo de Veículo</h2>
          {Object.keys(byType).length === 0 ? (
            <p className="text-sm text-blue-600">Sem dados.</p>
          ) : (
        ...
        <div className="glass rounded-2xl p-5">
          <h2 className="font-bold text-white mb-4">Top 5 — Mais Fretes</h2>
          <div className="space-y-2">
            {all.slice(0, 5).map(p => (
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <h2 className="font-bold text-white">Todos os Prestadores</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr style={{ background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                {['Nome','Veículo','Placa','Status','Fretes','Avaliação',''].map(h => (
            ...
            <tbody>
              {all.map(p => (
                <tr key={p.id} className="glass-hover transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/prestadores/${p.id}`} className="text-xs font-bold text-blue-400 hover:text-blue-300">Ver →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
