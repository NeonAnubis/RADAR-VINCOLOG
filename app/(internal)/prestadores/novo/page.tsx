  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-white">{t('providers.newTitle')}</h1>
          <p className="text-sm text-blue-400">{t('providers.newSubtitle')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className={L}>{t('providers.plateField')}</label><input name="vehiclePlate" required placeholder="BRA-2E19" className="glass-input" /></div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/prestadores" className="px-4 py-2.5 rounded-xl text-sm font-semibold text-blue-400 hover:text-white glass">{t('common.cancel')}</Link>
          <button type="submit" disabled={pending}
            ...
            {pending ? t('providers.creating') : t('providers.create')}
          </button>
        </div>
      </form>
    </div>
