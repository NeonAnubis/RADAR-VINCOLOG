...
interface Props { all: DbProvider[]; ativos: DbProvider[]; adormecidos: DbProvider[] }

export default function PrestadoresListView({ all, ativos, adormecidos }: Props) {
  const t = useT()

  const stats: Array<{ key: string; v: number; color: string; bg: string }> = [
    { key: 'active',       v: ativos.length,      color:'text-emerald-300', bg:'rgba(52,211,153,0.18)' },
    { key: 'dormant',      v: adormecidos.length, color:'text-blue-400',    bg:'rgba(255,255,255,0.07)' },
    { key: 'freightsDone', v: all.reduce((s,p)=>s+p.total_fretes,0), color:'text-sky-300', bg:'rgba(56,189,248,0.18)' },
  ]
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">{t('providers.title')}</h1>
          <p className="text-blue-400 mt-0.5 text-sm">{t('providers.registered', { count: all.length })}</p>
        </div>
        <Link href="/prestadores/novo"
          ...
          <Plus className="w-4 h-4" /> {t('providers.addShort')}
        </Link>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4">
        {stats.map(({ key, v, color, bg }) => (
          <div key={key} className="glass rounded-2xl p-5 flex items-center gap-4">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: bg }}>
              <Truck className={`w-5 h-5 ${color}`} />
            </div>
            <div>
              <p className="text-3xl font-extrabold text-white">{v}</p>
              <p className="text-xs text-blue-400 mt-0.5">{t(`providers.statsLabels.${key}`)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div>
        <h2 className="text-sm font-bold text-blue-300 mb-3 flex items-center gap-2 uppercase tracking-wider">
          ...
          {t('providers.active')} ({ativos.length})
        </h2>
              ...
              <div className="glass glass-hover rounded-2xl p-5 cursor-pointer transition-all">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="rounded-xl p-2.5 text-center" style={{ background:'rgba(255,255,255,0.05)' }}>
                    <p className="text-[10px] text-blue-400 uppercase tracking-wider">{t('providers.freights')}</p>
                    <p className="text-xl font-extrabold text-white mt-0.5">{p.total_fretes}</p>
                  </div>
                  <div className="rounded-xl p-2.5 text-center" style={{ background:'rgba(255,255,255,0.05)' }}>
                    <p className="text-[10px] text-blue-400 uppercase tracking-wider">{t('providers.rating')}</p>
                ...
                {!p.contract_signed && (
                  <div className="mt-2 px-2 py-1 rounded-lg text-[10px] font-semibold text-amber-300"
                    ...
                    {t('providers.contractPending')}
                  </div>
                )}
              </div>
            </Link>
          ))}
          {ativos.length === 0 && <p className="text-blue-600 text-sm col-span-3">{t('providers.noActive') !== 'providers.noActive' ? t('providers.noActive') : '—'}</p>}
        </div>
      </div>
      ...
      {adormecidos.length > 0 && (
        <div>
          <h2 className="text-sm font-bold text-blue-400 mb-1 flex items-center gap-2 uppercase tracking-wider">
            ...
            {t('providers.dormant')} ({adormecidos.length})
          </h2>
          <p className="text-xs text-blue-500 mb-3">{t('providers.dormantHelp')}</p>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {adormecidos.map(p => (
              <Link key={p.id} href={`/prestadores/${p.id}`}>
                <div className="glass-sm rounded-2xl p-5 cursor-pointer hover:glass transition-all opacity-75 hover:opacity-100">
                  ...
                  <div className="mt-3 pt-3 flex items-center justify-between text-xs text-blue-500"
                    ...
                    <span>{p.total_fretes} {t('providers.freights').toLowerCase()}</span>
                    <span>{t('providers.lastFreight')}: {fmtDate(p.updated_at)}</span>
                  </div>
                </div>
              </Link>
            ))}
