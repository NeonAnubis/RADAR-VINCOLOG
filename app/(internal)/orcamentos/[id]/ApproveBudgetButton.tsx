...
export default function ApproveBudgetButton({ budgetId, offered }: {
  ...
  function handleApprove() {
    if (!selected) { setError(t('budgetDetail.approveModal.selectError')); return }
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      ...
      <CheckCircle className="w-4 h-4" /> {t('budgetDetail.registerApproval')}
    </button>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(2,8,28,0.7)', backdropFilter: 'blur(4px)' }}>
      <div className="glass-strong rounded-2xl p-6 max-w-md w-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-extrabold text-white">{t('budgetDetail.approveModal.title')}</h2>
          <button onClick={() => setOpen(false)} className="text-blue-500 hover:text-white"><X className="w-4 h-4" /></button>
        </div>
        <p className="text-sm text-blue-300 mb-4">{t('budgetDetail.approveModal.instruction')}</p>
            ...
            return (
              <button key={k} onClick={() => setSelected(k)}
                ...
                <div className="text-left">
                  <p className="text-sm font-bold" style={{ color: meta.color }}>{t(`serviceLevels.${k}`)}</p>
                  {cfg.validity && <p className="text-[10px] text-blue-500">{t('budgetDetail.approveModal.validity')} {cfg.validity}</p>}
                </div>
        ...
        <div className="mb-3">
          <label className="block text-xs font-bold text-blue-300 mb-1 uppercase tracking-wider">{t('budgetDetail.approveModal.evidence')}</label>
          <input value={evidence} onChange={e => setEvidence(e.target.value)} placeholder={t('budgetDetail.approveModal.evidencePlaceholder')} className="glass-input" />
        </div>
        {error && <p className="text-xs text-red-400 mb-3">{error}</p>}
        <p className="text-xs text-blue-500 mb-4">{t('budgetDetail.approveModal.warning')}</p>
        <div className="flex gap-2">
          <button onClick={() => setOpen(false)} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-blue-300 glass">{t('common.cancel')}</button>
          <button onClick={handleApprove} disabled={pending || !selected}
            ...
            {t('budgetDetail.approveModal.submit')}
          </button>
        </div>
      </div>
    </div>
