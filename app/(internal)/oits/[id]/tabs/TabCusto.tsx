...
import { useSaveCommercial, useUpdatePaymentStatus } from '@/lib/hooks/use-oit-mutations'
...
export default function TabCusto({ oit }: { oit: DbOit }) {
  const t = useT()
  const saveCommercial = useSaveCommercial(oit.id)
  const pending = saveCommercial.isPending
  ...
  function handleSave() {
    saveCommercial.mutate({
      contracted_value: contracted, advance_amount: advance, balance_amount: balance,
      pedagio: num(data.pedagio), vale_pedagio: num(data.vale_pedagio), seguro: num(data.seguro),
      ciot: data.ciot || undefined, other_expenses: num(data.other_expenses),
    ...
    <div className="space-y-4">
      <div className="glass rounded-2xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-white flex items-center gap-2"><DollarSign className="w-4 h-4" /> {t('oits.costTab.title')}</h2>
          <div className="text-right">
            <p className="text-[10px] text-blue-500 uppercase tracking-widest">{t('oits.costTab.valueSoldToClient')}</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div><label className={L}>{t('oits.costTab.insurance')}</label>
        ...
        <div className="grid grid-cols-3 gap-3 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="p-4 rounded-xl" style={{ background: margin >= 0 ? 'rgba(52,211,153,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${margin >= 0 ? 'rgba(52,211,153,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
            <p className="text-[10px] uppercase" style={{ color: margin >= 0 ? '#34D399' : '#F87171' }}>{t('oits.costTab.grossMargin')}</p>
          ...
          <div className="p-4 rounded-xl flex items-center gap-2" style={{ background: marginPct >= 10 ? 'rgba(52,211,153,0.1)' : 'rgba(245,158,11,0.1)', border: `1px solid ${marginPct >= 10 ? 'rgba(52,211,153,0.3)' : 'rgba(245,158,11,0.3)'}` }}>
            ...
            <div>
              <p className="text-[10px] uppercase" style={{ color: marginPct >= 10 ? '#34D399' : '#FBBF24' }}>{t('oits.costTab.marginPct')}</p>
        ...
        {marginPct < 5 && contracted > 0 && (
          <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)' }}>
            ...
            <p className="text-xs text-red-300">{t('oits.costTab.marginAlert')}</p>
          </div>
        )}
        ...
        <button onClick={handleSave} disabled={pending}
          ...
          {pending ? t('oits.costTab.saving') : t('oits.costTab.save')}
        </button>
      </div>
...
function PaymentStatusBlock({ oit }: { oit: DbOit }) {
  const t = useT()
  const updateStatus = useUpdatePaymentStatus(oit.id)
  const pending = updateStatus.isPending
  const current = oit.payment_status ?? 'nao_iniciado'
  const cfg = PAYMENT_STATUSES[current]

  function move(s: PaymentStatus) { updateStatus.mutate(s) }

  return (
    <div className="glass rounded-2xl p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-bold text-white flex items-center gap-2"><CreditCard className="w-4 h-4" /> {t('oits.costTab.paymentTitle')}</h2>
      ...
      <div className="rounded-xl p-3 text-xs" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
        <p className="text-amber-300 font-bold mb-1">{t('oits.costTab.paymentRule')}</p>
        <p className="text-blue-300">{t('oits.costTab.paymentRuleDesc')}</p>
      </div>
    </div>
  )
}
